...
function UpdateSmoothedMovementDirection ()
{
	...
	if (grounded)
	{
		...
		// Pick speed modifier
		if (_padEmu.GetKey (KeyCode.LeftShift) || _padEmu.GetKey (KeyCode.RightShift))
		{
...
function ApplyGravity ()
{
	...
	{
		// Apply gravity
		var jumpButton = _padEmu.GetButton("Jump");
...
function Update() {
	if (!isControllable)
	{
		...
		_padEmu.ResetInputAxes();
	}

	if (_padEmu.GetButtonDown ("Jump"))
	{
...
{
	return Mathf.Abs(_padEmu.GetAxisRaw("Vertical")) + Mathf.Abs(_padEmu.GetAxisRaw("Horizontal")) > 0.5;
}
